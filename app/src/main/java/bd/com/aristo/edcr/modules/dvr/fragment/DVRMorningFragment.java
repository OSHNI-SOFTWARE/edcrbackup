...
public class DVRMorningFragment extends Fragment implements DVRSelectionListener{
            ...
            public boolean onClick(View v, IAdapter<IDoctorsModel> adapter, final IDoctorsModel item, int position) {
                if(!DVRUtils.isApproved){
                    if(item.isClicked()){
                        ...
                        //DVRActivity.activity.onRemove(true, item);
                    } else {
                        item.setClicked(true);
                        //DVRActivity.activity.onAdd(true, item);
                    }
                }
                adapter.getFastAdapter().notifyDataSetChanged();
                fastAdapter.getItemAdapter().filter("");
                return false;
            }
        });
    ...
    @Override
    public void onSelect(List<IDoctorsModel> iDoctorsModels) {
        dvrIDoctors.clear();
        dvrIDoctors.addAll(iDoctorsModels);
