...
public class  WPViewPager extends Fragment implements WPListener {
        ...
        public Fragment getItem(int position) {
            ...
            switch (position) {
                ...
                case 1:
                    ...
                    fragment.setArguments(bundle);
                    return fragment;
                    //return SampleFragment.newInstance(dateModel, wpUtilsModel);
                case 2:
                    fragment = new GiftFragment();

                    fragment.setArguments(bundle);
                    return fragment;
                    //return GWDSGiftFragment.newInstance(dateModel, wpUtilsModel);
                default:
                    return null;
    ...
    public void save(){
        if(isEligible()) {
            if(DVRUtils.isApprovedDayWise(r, dateModel.getMonth(), dateModel.getYear(), dateModel.getDay())) {
                ToastUtils.shortToast(StringConstants.SAVED_SUCCESS_MSG);
                saveWP();
            } else {
                ToastUtils.displayAlert(((Activity) context), "DVR is not Approved yet!! If DVR is not approved plan not allowed after 5th on every month.");
            }
        } else {
    ...
    public boolean isEligible(){
        ...
        if(cal.get(Calendar.DAY_OF_MONTH) == dateModel.getDay() && cal.get(Calendar.MONTH)+1 == dateModel.getMonth()){
            //Today 9:30 am
            cal.set(dateModel.getYear(), dateModel.getMonth() -1, dateModel.getDay(), 9, 30, 0);
        } else {
