...
import bd.com.aristo.edcr.models.db.NewDoctorModel;
...
public class NewDoctorFragment extends Fragment implements NewDCRListener {
    ...
    List<String> autoSuggestNameList = new ArrayList<>();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        View rootView                                   = inflater.inflate(R.layout.new_dcr_new_doctor_view_pager, container, false);
        ButterKnife.bind(this, rootView);
        getUserInfo();
        ((Activity) context).setTitle("New Doctor");
    ...
    public void getSuggestName(){
        if (autoSuggestNameList.size()>0){
            autoSuggestNameList.clear();
        }
        RealmQuery<NewDoctorModel> query = r.where(NewDoctorModel.class);
        RealmResults<NewDoctorModel> autoSuggestList = query.findAll();

        for (NewDoctorModel addressModel:autoSuggestList){
            autoSuggestNameList.add(addressModel.getName());
        }

        ArrayAdapter<String> adapter =
                new ArrayAdapter<String>(context, android.R.layout.simple_list_item_1, autoSuggestNameList);
        ...
        if(isValidate()) {
            r.executeTransaction(new Realm.Transaction() {
                ...
                public void execute(Realm r) {
                    ...
                    newDCRModel.setDoctorID(StringUtils.getAndFormAmp(etName.getText().toString()));
                    ...
                    //Parent data
                    //NewDoctorModel newDoctorModel = new NewDoctorModel();
                    NewDoctorModel newDoctorModel =r.where(NewDoctorModel.class).equalTo("name",StringUtils.getAndFormAmp(etName.getText().toString())).findFirst();
                    if (newDoctorModel==null){
                        r.insert(new NewDoctorModel(StringUtils.getAndFormAmp(etName.getText().toString())));
                    }
 ...
 public void  newDCRUpload(){
     ...
     dcrSendModel.setDoctorId(StringUtils.getAndFormAmp(etName.getText().toString()));
