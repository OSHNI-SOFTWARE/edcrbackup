...
public class PromotedFragment extends Fragment {
    ...
    private void refreshList(){
        List<WPProductsModel> productsModels = WPUtils.getPromotedProducts(r, dateModel, wpUtilsModel, WPViewPager.wpViewPager);
            ...
            fastAdapter.withItemEvent(new ClickEventHook<WPProductsModel>() {
                ...
                public void onClick(View v, int position, FastAdapter<WPProductsModel> fastAdapter1, final WPProductsModel item) {
                    ...
                    if (item.getQuantity() >= item.getBalance()) {
                        ...
                        WPViewPager.wpViewPager.addProduct(item.getCode(), 0, item.getCount(), item.getName());
