...
public class GWDSListFragment extends Fragment {
    ...
    public void setTitle(){
        ((Activity) context).setTitle("GWDS for "+DateTimeUtils.getFullMonthForInt(dateModel.getMonth()));
    }
       ...
       final FastItemAdapter<PWDSReport> fastAdapter = new FastItemAdapter<>();
        ...
        fastAdapter.withOnClickListener(new FastAdapter.OnClickListener<PWDSReport>() {
            ...
            public boolean onClick(View v, IAdapter<PWDSReport> adapter, PWDSReport item, int position) {
                GWDSUtilsModel pwdsUtilsModel = new GWDSUtilsModel(item.getpCode(), item.getProductName(), item.getCount(), position);
                Fragment fragment = new DoctorsFragment();
                Bundle b = new Bundle();
                b.putSerializable("gwdsUtils", pwdsUtilsModel);
    ...
    public List<PWDSReport> getPWDSReport(){
        ...
        List<ProductModel> productModelList = GWDSUtils.getGWDSProducts(r, dateModel.getYear(), dateModel.getMonth());
        int i = 0;
    ...
    public String getPWDSDoctors(String code){
        ...
        List<GWDSModel> pwdsModels = r.where(GWDSModel.class)
                .equalTo(GWDSModel.COL_GIFT_ID, code)
        ...
        if(pwdsModels != null && pwdsModels.size() > 0){
            int i = 1;
            String prefix = "";
            for(GWDSModel pwdsModel:pwdsModels){
                String doctorName = "";
