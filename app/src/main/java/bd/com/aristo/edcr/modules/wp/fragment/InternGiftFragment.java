...
public class InternGiftFragment extends Fragment {
    ...
    private void refreshList(){
        List<WPProductsModel> productsModels = WPUtils.getGiftProductsForIntern(r, dateModel, wpUtilsModel, WPInternViewPager.wpViewPager);
        if(productsModels.size() > 0) {
            ...
            fastAdapter.withItemEvent(new ClickEventHook<WPProductsModel>() {
                ...
                public void onClick(View v, int position, FastAdapter<WPProductsModel> fastAdapter1, final WPProductsModel item) {
                    ...
                    if (item.getQuantity() >= item.getBalance()) {
                        ...
                        WPInternViewPager.wpViewPager.addProduct(item.getCode(), 2, item.getCount(), item.getName());
            ...
            bundle.putString(StringConstants.SAMPLE_EMPTY_DIALOG_TITLE, "Intern Gift Empty");
            bundle.putString(StringConstants.SAMPLE_EMPTY_DIALOG_MSG, "There is no gift for Intern, please sync!!");
